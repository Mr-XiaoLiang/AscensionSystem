package com.lollipop.ascensionsystem.util.banner
...
class LBannerLayoutManager: RecyclerView.LayoutManager(),RecyclerView.SmoothScroller.ScrollVectorProvider{
    ...
    private fun layoutItems(recycler: RecyclerView.Recycler, state: RecyclerView.State) {
        ...
        selectedPosition = if(selectedIndex < 0){
            ...
            getChildAt(selectedIndex)?.let { getPosition(it) }?: -1
        }
    ...
    override fun scrollToPosition(position: Int) {
        ...
        val pos = min(max(position, 0),itemCount)
        offset = if(orientation == Orientation.HORIZONTAL){
            itemsFrames.get(pos)?.left?:0
        }else{
            itemsFrames.get(pos)?.top?:0
        }
    ...
    fun onSelectedChange(lis: ((position: Int,state: ScrollState) -> Unit)?) {
        onSelectedListener = if (lis == null) {
            null
        } else {
